...
u16 CST816S_GetX (void) {
    ...
    CST816S_RD_DATA (GT_ADDR_REG, 1, &mode);   // 读取触摸模式数据

    if (mode >= 1){  // 检测到触摸
        CST816S_RD_DATA (TPX[0], 4, buf);      // 读取第一个触摸点的坐标数据

        // 根据横屏或竖屏模式转换坐标
        if (USE_HORIZONTAL == 2) {
            x_coord = buf[3];        // 横屏
        } else if (USE_HORIZONTAL == 0) {
            x_coord = 240 - buf[1];  // 竖屏模式 0
        } else if (USE_HORIZONTAL == 1) {
            x_coord = buf[1];        // 竖屏模式 1
        } else {
            x_coord = 240 - buf[3];  // 竖屏模式 2
        }
    }
...
u16 CST816S_GetY (void) {
    ...
    if (mode >= 1){  // 检测到触摸
        ...
        if (USE_HORIZONTAL == 2) {
            ...
            y_coord = buf[3];        // 竖屏模式 0
        } else if (USE_HORIZONTAL == 1) {
            y_coord = 240 - buf[3];  // 竖屏模式 1
        } else {
            y_coord = 240 - buf[1];  // 竖屏模式 2
        }
    }
...
u16* CST816S_GetXY(void) {
    static u16 coords[2];  // 用静态数组存储返回的坐标
    u8 buf[4];
    u8 mode;
    ...
    if (mode >= 1) {  // 检测到触摸
        ...
        if (USE_HORIZONTAL == 2) {
            ...
            coords[0] = 240 - buf[1];  // 竖屏模式 0
        } else if (USE_HORIZONTAL == 1) {
            coords[0] = buf[1];  // 竖屏模式 1
        } else {
            coords[0] = 240 - buf[3];  // 竖屏模式 2
        }
        ...
        if (USE_HORIZONTAL == 2) {
            ...
            coords[1] = buf[3];  // 竖屏模式 0
        } else if (USE_HORIZONTAL == 1) {
            coords[1] = 240 - buf[3];  // 竖屏模式 1
        } else {
            coords[1] = 240 - buf[1];  // 竖屏模式 2
        }
    }

    return coords;  // 返回坐标数组
}
